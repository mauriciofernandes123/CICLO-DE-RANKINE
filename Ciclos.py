...
class Ciclos:
    ...
    def rankineSimples(op1, p1, t1, p2, op2, x3, t3):
        ...
        try:
            ...
            elif  t3<t4 or t3<t2: #Erro no estado 3
                erro= 6

            elif  s4<s1 or t4>t3: #Erro no estado 3
                erro= 7
...
#Se tiver algum valor vazio vai entrar na condição verificar1 = 2 que vai acenar um alerta no front
            ...
            return {'eta': eta, 'wt': wt/1000, 'wb': wb/1000, 'qh': qh/1000, 'erro': erro}
        
        except ValueError:
            verificar1 = 2
            print(f"o valor do erro{verificar1}")
            if verificar1 ==2:
               
                return verificar1
           
    def reaquecimento(op1, p1, t1, p2, op2, x3, t3, p4, op3, t5, x5, grafico):
        ...
        try:
            ...
            if eta <= 0 or qh<=0 or wt<=0 or wb>=0:
                ...
                if h3<=h2_iso or h5<=h4: #erro na caldeira
                    erro =1

                elif h4>=h3 or h6>=h5: #Erro na turbina
                    erro =2

                elif h1>=h2_iso: #erro na bomba
                    erro = 3

            elif p4 < p1 or p4> p2: #Erro na pressão intermediária
                    erro =4
            ...
            elif s6<s1 or s6<s4 or t6>t5: #Erro do estado 6
                erro = 9
        ...
        #__________verifica as propriedades dentro da tabela pela entropia___________________
            ...
            return {'n': eta, 'wt':wt/1000, 'wb': wb/1000, 'qh': qh/1000, 'erro':erro, 'wliq':wliq}
    ...
    def regenerativo(op1, p1, t1, p2, op2, p5, t5, x5, op3, y):
        ...
        # op3 = int(input("A segunda bomba depende de: \n 1- Entalpia da primeira lei do misturador \n 2-Título"))
            ...
            # y = float(input('Entre com y: ')) 
                ...
                h4 = h3 + v3*(p5 - p3)
                y= (h3-h2)/(h6-h2)
                s4 = 1/CP.PropsSI('D', 'P', p4, 'H', h4, fluid) 
            ...
            if eta <= 0 or qh<=0 or wt<=0 or wb>=0:
                ...
                if h5<=h4: #erro na caldeira
                    erro =1

                elif h7>=h6 or h6>=h5: #Erro na turbina
                    erro =2

                elif h1>=h2 or h3>h4: #erro na bomba
                    erro = 3
            ...
            elif t4< t3 or t4>t5: #erro no estado 4
                erro = 7
            ...
            elif s7<s2 or t7>t6 or t7>t5: #erro no estado 7
                erro= 10    
    ...
    #__________verifica as propriedades dentro da tabela pela entropia___________________
            ...
            return {'eta':eta, 'wt':wt/1000, 'wb':wb/1000, 'qh':qh/1000,'y': y, 'erro':erro}
    ...
    def regenerativoReaquecimento(m, op1, p1, t1, p2, n, op3, p3, t3, op5, t5, x5, p6, nt, op7, t7, x7 ):
        ...
        # op7 = int(input("A turbina de baixa pressão depende de: \n 1-Pressão e temperatura \n 2-Pressão e titulo \n "))  # entrada da condição da turbina de alta pressão
            ...
            t8 = CP.PropsSI('T', 'P', p8, 'H', h8, fluid) - 273.15
    ...
    #__________verifica as propriedades dentro da tabela pela entropia___________________
            ...
            return {'eta':eta, 'wt':wt/1000, 'wb':wb/1000, 'qh': qh/1000, 'y':y, 't8': t8, 'm8': m8, 'wliq': wliq/1000, 'erro':erro }
